...
struct proc * scheduler(struct proc * current)
{
    ...
    // Pegando o maior processo da fila de aptos para executar
    selected = get_big_process(ready);
...
struct proc * get_big_process(struct queue * ready){
    ...
    if (p_big_time!= NULL){
        dequeue_bypid(ready, p_big_time->pid);
    }
